                    ...
                    <div class="row">
                        ...
                        <div class="col-7">
                            <div class="detail">
                                <div class="top">
                                    <div class="date">
                                        <span style="color: brown;" class="name">{{ $detail -> name }}</span><br>
                                        @if($detail-> sale_price < 0 || $detail-> sale_price == 0)
                                            <div class="regular-price1">US {{ $detail->regular_price }}</div>
                                        @endif
                                        @if ($detail-> sale_price > 0)
                                            <div class="regular-price"><strike>US {{ $detail->regular_price }}</strike></div>
                                            <div class="sale-price1">% -{{ number_format(($detail-> sale_price * 100)  / $detail -> regular_price,2) }}</div>
                                        @endif
                                    ...
                                    <div class="detail">
                                        <div class="price-list">
                                            <div class="price d-none">US 10</div>
                                                @if($relatedProduct-> sale_price < 0 || $relatedProduct-> sale_price == 0)
                                                    <div class="regular-price">US {{ $relatedProduct->regular_price }}</div>
                                                @endif
                                                @if ($relatedProduct-> sale_price > 0)
                                                    <div class="regular-price"><strike>US {{ $relatedProduct->regular_price }}</strike></div>
                                                    <div class="sale-price ">% -{{ number_format(($relatedProduct-> sale_price * 100)  / $relatedProduct -> regular_price,2) }}</div>
                                                @endif
                                        </div>
